...
PYTHON = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "prism", ".venv", "bin", "python")
...
path = "prism/results/diag_{name}.json"
...
def load_extracted():
    ...
    path = os.path.join(PROJECT_ROOT, "prism/results/extracted_spectra.json")
    with open(path) as f:
        data = json.load(f)
    return data
...
def main():
    ...
    for name, method, steps, lr, warmup, seed, kwargs in tests:
        result_path = os.path.join(PROJECT_ROOT, f"prism/results/diag_{name}.json")
    ...
    print(f"  Results in prism/results/diag_*.json", flush=True)
    print(f"{'='*60}", flush=True)

    # Touch sentinel
    with open(os.path.join(PROJECT_ROOT, "prism/results/.diag_done"), "w") as f:
