...
path = "prism/results/lr_sweep_{name}.json"
...
def main():
    ...
    for name, method, lr, kwargs in tests:
        result_path = f"prism/results/lr_sweep_{name}.json"
    ...
    for lr_name, lr in lrs.items():
        ext_path = f"prism/results/lr_sweep_extracted_{lr_name}_s42.json"
        ort_path = f"prism/results/lr_sweep_ortho_{lr_name}_s42.json"
        ext_ppl = "?"
        ort_ppl = "?"
