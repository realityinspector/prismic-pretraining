...
Usage:
    python -m prism.run test              # Quick smoke test (20 steps, <30s)
    python -m prism.run baselines         # Run all baseline inits (5K steps each)
    python -m prism.run baselines --quick # Quick baselines (200 steps each)
    python -m prism.run extract           # Extract spectra from pretrained GPT-2
    python -m prism.run search            # CMA-ES spectral search
    python -m prism.run eval [genome]     # Full eval with optional IMT genome
    python -m prism.run plot              # Generate plots from saved results
"""
import sys
import os
...
def cmd_test():
    ...
    from prism.config import TrainConfig, get_device
    from prism.train import train
...
def cmd_baselines(quick=False):
    ...
    from prism.config import TrainConfig, get_device
    from prism.eval import run_baselines
...
def cmd_search():
    ...
    from prism.config import TrainConfig, SearchConfig, get_device
    from prism.search import run_search
...
def cmd_eval(genome_path=None, extracted=False, quick=False):
    ...
    from prism.config import TrainConfig, get_device
    from prism.eval import run_all
    ...
    extracted_path = "prism/results/extracted_spectra.json" if extracted else None
...
def cmd_extract():
    ...
    """
    from prism.spectral_init import classify_param, ATTN_PROJ, FFN_UP, FFN_DOWN, EMBED
    import torch
    import numpy as np
    import json
    import os
    ...
    # Save
    os.makedirs("prism/results", exist_ok=True)
    result = {
    ...
    path = "prism/results/extracted_spectra.json"
    with open(path, "w") as f:
        json.dump(result, f, indent=2)

    print(f"\nSaved to {path}")
    print("Use with: python -m prism.run eval --extracted")


def cmd_plot():
    """Generate plots from saved results."""
    from prism.plot import plot_all_from_dir
    plot_all_from_dir()
