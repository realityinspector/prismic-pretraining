...
Usage:
    python -m prism.stairclimb              # run next untested hypothesis
    python -m prism.stairclimb --list       # show all hypotheses + results
    python -m prism.stairclimb --run NAME   # run specific hypothesis
"""
import json
import sys
import os
...
PYTHON = os.path.join(PROJECT_ROOT, "prism", ".venv", "bin", "python")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "prism", "results")
...
from prism.config import TrainConfig, get_device, install_signal_handlers, set_process_memory_limit
...
# Save
path = "prism/results/sc_{name}.json"
...
from prism.spectral_init import apply_spectral_init, ATTN_PROJ, FFN_UP, FFN_DOWN, EMBED, classify_param
...
''',
    ...
    (
        ...
        '''
from prism.pretrained_extract import extract_per_layer, make_per_layer_init_fn
...
''',
    ...
    (
        ...
        '''
from prism.spectral_unfold import seed_from_pretrained, make_unfolded_init_fn
...
''',
    ...
    (
        ...
        '''
from prism.spectral_unfold import seed_from_pretrained, make_unfolded_init_fn
from prism.pretrained_extract import extract_per_layer
meta = seed_from_pretrained("gpt2")
dirs = extract_per_layer("gpt2", include_directions=True)
...
from prism.spectral_init import classify_param, ATTN_PROJ, FFN_UP, FFN_DOWN, EMBED, apply_spectral_init
